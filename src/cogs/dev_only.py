...
class DevOnly(commands.Cog):
    ...
    # Logging channel management (Developer only)
    @commands.command(name="setlogchannel", description="Set logging channels for the bot")
    async def set_log_channel(self, ctx, log_type: str = None, channel: discord.TextChannel = None):
        """Set logging channels for bot operations"""
        if not self.is_owner_or_authorized(ctx):
            await self.unauthorized_response(ctx)
            return
        
        if not log_type or log_type.lower() not in ['cmd', 'dm', 'guild']:
            embed = discord.Embed(
                title="Invalid Log Type",
                description="Please specify a valid log type:\n\n"
                           "**Available types:**\n"
                           "`cmd` - Command logging channel\n"
                           "`dm` - DM logging channel\n"
                           "`guild` - Guild join/leave logging channel",
                color=EMBED_COLOR_ERROR
            )
            await ctx.reply(embed=embed, mention_author=False)
            return
        ...
        if not channel:
            embed = discord.Embed(
                ...
                description="Please specify a channel to set as the logging channel.\n\n"
                           "**Usage:** `s.setlogchannel <type> <#channelID>`\n"
                           "**Example:** `s.setlogchannel cmd #bot-logs`",
                color=EMBED_COLOR_ERROR
            )
            await ctx.reply(embed=embed, mention_author=False)
            return
        ...
        # Store the channel ID in the appropriate logging system
        if log_type == 'cmd':
            cmd_logging_cog = self.bot.get_cog('CommandLogging')
            if cmd_logging_cog:
                cmd_logging_cog.set_log_channel(ctx.guild.id, channel.id)
        elif log_type == 'dm':
            dm_logging_cog = self.bot.get_cog('DMLogging')
            if dm_logging_cog:
                dm_logging_cog.set_log_channel(ctx.guild.id, channel.id)
        elif log_type == 'guild':
            events_cog = self.bot.get_cog('Events')
            if events_cog:
                events_cog.set_log_channel(ctx.guild.id, channel.id)
        
        embed = discord.Embed(
            title="Logging Channel Set",
            description=f"**{log_type.upper()}** logging channel set to {channel.mention}",
            color=EMBED_COLOR_NORMAL
        )
        embed.add_field(
            name="Channel Info",
    ...
    @commands.command(name="getlogchannels", description="View current logging channel settings")
    async def get_log_channels(self, ctx):
        """View current logging channel settings"""
        if not self.is_owner_or_authorized(ctx):
            await self.unauthorized_response(ctx)
            return
        ...
        embed = discord.Embed(
            ...
            description="Current logging channel configuration for this server",
            color=EMBED_COLOR_NORMAL
        )
        
        # Get command logging channel
        cmd_logging_cog = self.bot.get_cog('CommandLogging')
        cmd_channel = "Not set" if not cmd_logging_cog else cmd_logging_cog.get_log_channel(ctx.guild.id)
        if cmd_channel and cmd_channel != "Not set":
            channel_obj = self.bot.get_channel(cmd_channel)
            cmd_channel = channel_obj.mention if channel_obj else f"Channel ID: {cmd_channel} (not found)"
        
        # Get DM logging channel
        dm_logging_cog = self.bot.get_cog('DMLogging')
        dm_channel = "Not set" if not dm_logging_cog else dm_logging_cog.get_log_channel(ctx.guild.id)
        if dm_channel and dm_channel != "Not set":
            channel_obj = self.bot.get_channel(dm_channel)
            dm_channel = channel_obj.mention if channel_obj else f"Channel ID: {dm_channel} (not found)"
        
        # Get guild logging channel
        events_cog = self.bot.get_cog('Events')
        guild_channel = "Not set" if not events_cog else events_cog.get_log_channel(ctx.guild.id)
        if guild_channel and guild_channel != "Not set":
            channel_obj = self.bot.get_channel(guild_channel)
            guild_channel = channel_obj.mention if channel_obj else f"Channel ID: {guild_channel} (not found)"
        ...
        embed.add_field(
            name="Management",
            value="Use `s.setlogchannel <type> <#channelID>` to change settings",
            inline=False
        )
    ...
    @commands.command(name="blacklist", description="Blacklist a user", hidden=True)
    @commands.is_owner()
    async def blacklist_user(self, ctx, user: discord.User, *, reason: str = "No reason provided"):
        ...
        try:
            ...
            logger.info(f"User {user} blacklisted by {ctx.author}: {reason}")
            
        except Exception as e:
            embed = discord.Embed(
                title="Error",
                description=f"Error blacklisting user: {str(e)}",
                color=EMBED_COLOR_ERROR
            )
            embed.set_footer(text=f"Requested by {ctx.author.display_name}", icon_url=ctx.author.display_avatar.url)
            embed.timestamp = discord.utils.utcnow()
    ...
    @commands.command(name="unblacklist", description="Remove user from blacklist", hidden=True)
    @commands.is_owner()
    async def unblacklist_user(self, ctx, user: discord.User):
        ...
        try:
            ...
            logger.info(f"User {user} unblacklisted by {ctx.author}")
            
        except Exception as e:
            embed = discord.Embed(
                title="Error",
                description=f"Error unblacklisting user: {str(e)}",
                color=EMBED_COLOR_ERROR
            )
            embed.set_footer(text=f"Requested by {ctx.author.display_name}", icon_url=ctx.author.display_avatar.url)
            embed.timestamp = discord.utils.utcnow()
    ...
    async def devhelp(self, ctx):
        ...
        embed.add_field(
            ...
            value=(
                "`s.listdata` - List data files for users/guilds\n"
                "`s.deletedata` - Delete specific user/guild data\n"
