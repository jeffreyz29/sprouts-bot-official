...
from emojis import SPROUTS_ERROR
...
class DiscordBot(commands.AutoShardedBot):
    ...
    async def get_prefix(self, message):
        ...
        if message.guild:
            try:
                ...
                from cogs.guild_settings import guild_settings
    ...
    async def setup_hook(self):
        ...
        from database.replit_persistence import replit_persistence
        ...
        from cogs.uncategorized import setup_uncategorized
        from cogs.events import setup_events
        from cogs.tickets import setup as setup_tickets
        from cogs.utilities import setup_utilities
        from cogs.help import setup_help
        ...
        from cogs.logging.cmd_logging import setup_cmd_logging
        from cogs.logging.cluster import setup as setup_cluster
        from cogs.ticket_settings import TicketSettings
        from cogs.ticket_tags import TicketTags
        from cogs.staff_manager import StaffManager
        ...
        # Load TicketsBot-compatible cogs
        await self.add_cog(TicketSettings(self))
        await self.add_cog(TicketTags(self))
        await self.add_cog(StaffManager(self))
        
        # Add persistent views for tickets system
        try:
            from cogs.tickets import SproutsTicketView, SproutsCreateTicketView
            ticket_cog = self.get_cog('SPROUTS Tickets')
            if ticket_cog:
                self.add_view(SproutsTicketView(ticket_cog))
                self.add_view(SproutsCreateTicketView("persistent", ticket_cog))
                logger.info("Tickets persistent views added successfully")
    ...
    async def global_check(self, ctx):
        ...
        try:
            from cogs.dev_only import global_cooldown
